...
use crate::flow::{InterceptedRequest, InterceptedResponse, Scheme};
...
impl ScriptEngine {
    pub fn new(script_path: PathBuf) -> anyhow::Result<Self> {
        println!("Initializing Lua script engine with script: {script_path:?}",);
        let lua = Lua::new();
    ...
    pub fn register_functions(lua: &Lua) -> anyhow::Result<()> {
        ...
        let notify = lua.create_function(move |_, (_msg, _level): (String, Option<i32>)| {
            // match level {
    ...
    fn load_script(&self) -> anyhow::Result<()> {
        info!("Loading script {:?}", self.script_path);
        ...
        // notify_info!("Load script: {:?}", self.script_path);
        Ok(())
    }
    ...
    pub fn intercept_request(&self, req: &mut InterceptedRequest) -> anyhow::Result<()> {
        self.intercept_request_inner(req).map_err(|e| {
            error!("ScriptEngine {}", e);
            let script_code = fs::read_to_string(&self.script_path).unwrap();
            error!("script {}", script_code);
            e
        })
    }

    fn intercept_request_inner(&self, req: &mut InterceptedRequest) -> anyhow::Result<()> {
        let lua = self
            .lua
            .read()
        ...
        req_table
            .set("scheme", req.scheme.to_string())
            .map_err(|e| anyhow!("Failed to set 'scheme': {}", e.to_string()))?;
        ...
        req_table
            .set("port", req.port)
            .map_err(|e| anyhow!("Failed to set 'port': {}", e.to_string()))?;

        req_table
            .set("path", req.path.clone())
            .map_err(|e| anyhow!("Failed to set 'path': {}", e.to_string()))?;
        ...
        if let Ok(value) = new_req.get::<String>("scheme") {
            match value.as_str() {
                "https" => {
                ...
                _ => {
                    warn!("Invalid scheme from lua: {}", value);
                }
            }
        };
        ...
        req.path = new_req
            .get("path")
            .map_err(|e| anyhow!("Missing or invalid 'path' in Lua result: {}", e.to_string()))?;
    ...
    pub fn intercept_response(&self, res: &mut InterceptedResponse) -> anyhow::Result<()> {
        self.intercept_response_inner(res).map_err(|e| {
            error!("ScriptEngine {}", e);
            e
        })
    }

    fn intercept_response_inner(&self, res: &mut InterceptedResponse) -> anyhow::Result<()> {
        let lua = self
            .lua
            .read()
            .map_err(|e| anyhow!("Failed to acquire Lua lock: {e}"))?;
        ...
        for pair in new_headers.pairs::<String, String>() {
            let (k, v) = pair
                .map_err(|e| anyhow!("Invalid header entry in Lua result: {}", e.to_string(),))?;
            info!("header {}:{}", k, v);
