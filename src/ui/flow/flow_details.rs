...
use crate::{
    event::Action,
    flow::{
        CertInfo, FlowKind, FlowStore, InterceptedRequest, InterceptedResponse, Timing, WsMessage,
    },
    notify_info,
    ui::framework::{
...
impl FlowDetails {
    pub fn new(flow_store: FlowStore) -> Self {
        ...
        let (cert_tx, cert_rx) = mpsc::channel::<Option<Vec<CertInfo>>>(64);
...
async fn update_flow_view(
    ...
    cert_tx: &mpsc::Sender<Option<Vec<CertInfo>>>,
        ...
        if let Some(entry) = maybe_entry {
            ...
            match &flow.kind {
                ...
                FlowKind::Http2(flow) => {
                    ...
                    req_tx
                        .send(flow.requests.first().cloned())
                        .await
                        .unwrap_or_else(|e| {
                            error!("Failed to send request: {}", e);
                        });
                    ...
                    resp_tx
                        .send(flow.responses.first().cloned())
                        .await
                        .unwrap_or_else(|e| {
                            error!("Failed to send response: {}", e);
                        });
